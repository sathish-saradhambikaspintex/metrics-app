...
class PlaceHolder extends AbstractPlaceHolder {
	renderPlaceHolder() {
		...
		const inputElement = isArrayType ? (
			<Controls.Input.Multiple type="datetime-local" readOnly {...inputProps} />
		) : (
			<Controls.Input type="datetime-local" readOnly {...inputProps} />
		);
...
const TagIcon = <Tag.Icon key="datetime-local-tag" id="datetime-local" iconText="dd/mm/yy hh:mm" text="Date & Time" />;
