...
class PlaceHolder extends AbstractPlaceHolder {
	renderPlaceHolder() {
		...
		const inputElement = isArrayType ? (
			<Controls.Input.Multiple type="text" readOnly {...inputProps} />
		) : (
			<Controls.Input type="text" readOnly {...inputProps} />
		);
...
const TagIcon = <Tag.Icon key="text-tag" id="text" icon="font" text="Text" />;

export default { Field, PlaceHolder, TagIcon, getInitialState };
