...
class PlaceHolder extends AbstractPlaceHolder {
	renderPlaceHolder() {
		...
		const inputElement = isArrayType ? (
			<Controls.Input.Multiple type="date" readOnly {...inputProps} />
		) : (
			<Controls.Input type="date" readOnly {...inputProps} />
		);
...
const TagIcon = <Tag.Icon key="date-tag" id="date" icon="calendar-o" text="Date" />;
