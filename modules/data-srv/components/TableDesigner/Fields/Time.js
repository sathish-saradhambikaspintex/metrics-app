...
class PlaceHolder extends AbstractPlaceHolder {
	renderPlaceHolder() {
		...
		const inputElement = isArrayType ? (
			<Controls.Input.Multiple type="time" readOnly {...inputProps} />
		) : (
			<Controls.Input type="time" readOnly {...inputProps} />
		);
...
const TagIcon = <Tag.Icon key="time-tag" id="time" icon="clock-o" text="Time" />;
